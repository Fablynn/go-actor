...
import (
	"go-actor/common/pb"
	"go-actor/common/redis/internal/client"
...
func IncrBy(dbid string, key string, val int64) (ret int64, err error) {
	...
	if cli == nil {
		err = uerror.New(pb.ErrorCode_CLIENT_NOT_FOUND, "dbid:%s", dbid)
		return
	}
...
func HIncrBy(dbid string, key string, field string, incr int64) (int64, error) {
	cli := manager.GetRedis(dbid)
	if cli == nil {
		return 0, uerror.New(pb.ErrorCode_CLIENT_NOT_FOUND, "dbid:%s", dbid)
	}
