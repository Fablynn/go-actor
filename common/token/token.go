...
func cb(token *jwt.Token) (interface{}, error) {
	...
	if _, ok := token.Method.(*jwt.SigningMethodHMAC); !ok {
		return nil, uerror.New(pb.ErrorCode_TYPE_ASSERT_FAILED, "unexpected signing method: %v", token.Header["alg"])
	}
...
func ParseToken(tt string) (*Token, error) {
	...
	return nil, uerror.New(pb.ErrorCode_PARSE_FAILED, "token is invalid")
}
